...
def collect_tasks(config: OCRConfig) -> list[OCRTask]:
    """
    Scans the input directory, filters against existing results for resumability,
    and creates a final list of files to process.
    """
    print("--- Collecting and filtering tasks ---")
    
    # --- THIS IS THE RESUMABILITY LOGIC ---
    processed_paths = set()
    if not config.force_rerun:
        # Load the set of source_path strings from the output file
    ...
    # Use rglob to find all files recursively. Converting to a list here to show a total in tqdm.
    ...
    for file_path in tqdm(all_files_in_dir, desc="Filtering tasks"):
        ...
        # Skip if filename contains an ignore keyword
        if any(keyword in filename_lower for keyword in ignore_keywords_lower):
            continue
        
        # Add to task list if it's a supported file type
